...
//		mtlist(db);
	...
	private static void mtlist(SqlserverJdbc db2) throws Exception{
		String sql = "";
		String sql1 = "";
        ResultSet rs = null;
        ...
        try{
            sql = " select a.FItemID, a.FNumber, a.FName, a.FModel, cast(a.FQty as numeric(10,2)) as FQty, "
                ...
                + " from( "
                + "         select FItemID, FNumber, FName, FModel, sum(FQty) as FQty "
                + "         from #TmpErpMtInfo "
                + "         group by FItemID, FNumber, FName, FModel "
                + " ) a "
            ...
            while(rs.next()){
                ...
                /*String[] str ={
                    rs.getString("FItemID").trim(),
                    ""+(++index),
                    rs.getString("FNumber").trim(),
                    rs.getString("FName").trim(),
                    rs.getString("FModel").trim(),
                    rs.getString("CLQty").trim(),
                    rs.getString("DJQty").trim(),
                    rs.getString("TLFQty").trim(),
                    rs.getString("PlanNum").trim(),
                    (fnum.format(rs.getFloat("FQty")))+"",
                    (fnum.format(rs.getFloat("CLQty")+rs.getFloat("DJQty")-rs.getFloat("TLFQty")
                    		+rs.getFloat("PlanNum")-rs.getFloat("FQty")))+""
                };
                list.add(str);*/
                
            	
            	sb.append("{").append("\"FItemID\":\"").append(rs.getString("FItemID").trim()).append("\"").append(",")
				.append("\"FNumber\":\"").append(rs.getString("FNumber").trim()).append("\"").append(",")
				.append("\"FName\":\"").append(rs.getString("FName").trim()).append("\"").append(",")
				.append("\"FModel\":\"").append(rs.getString("FModel").trim()).append("\"").append(",")
				.append("\"CLQty\":\"").append(rs.getString("CLQty").trim()).append("\"").append(",")
				.append("\"DJQty\":\"").append(rs.getString("DJQty").trim()).append("\"").append(",")
				.append("\"TLFQty\":\"").append(rs.getString("TLFQty").trim()).append("\"").append(",")
				.append("\"PlanNum\":\"").append(rs.getString("PlanNum").trim()).append("\"").append(",")
				.append("\"TXqty\":\"").append(fnum.format(rs.getFloat("FQty")+rs.getFloat("TLFQty"))).append("\"").append(",")
				.append("\"TSqty\":\"").append(fnum.format(rs.getFloat("CLQty")+rs.getFloat("DJQty")-rs.getFloat("FQty")
						-rs.getFloat("TLFQty"))).append("\"").append(",");
				
            	
            	sql1 = " select b.FItemID, c.FNumber, b.des, b.Data_1, b.Data_2, a.DateSeq, a.ErpSeq, b.Data_seq "
                        + " from( "
                        ...
                        + " ) c on b.FItemID=c.FItemID where b.FItemID='"+rs.getString("FItemID").trim()+"'"
                        + " order by c.FNumber, b.FItemID, a.DateSeq, a.ErpSeq, b.Data_seq ";
            	ResultSet rs01 = db2.ExecuteQuery(sql1);
            	while (rs01.next()) {
            		sb.append(rs01.getString("Data_1")).append(",").append(rs01.getString("Data_2")).append(",");
//					System.out.println(rs01.getString("Data_1")+",,,,,,,,"+rs01.getString("Data_2"));
				}
            	sb.deleteCharAt(sb.length()-1);
            	sb.append("},");
...
//				dic.put("FItemID", rs.getString("FItemID").trim());
//				dic.put("FNumber", rs.getString("FNumber").trim());
//				dic.put("FName", rs.getString("FName").trim());
//				dic.put("FModel", rs.getString("FModel").trim());
//				dic.put("CLQty", rs.getString("CLQty").trim());
//				dic.put("DJQty", rs.getString("DJQty").trim());
//				dic.put("TLFQty", rs.getString("TLFQty").trim());
//				dic.put("PlanNum", rs.getString("PlanNum").trim());
//				dic.put("TXqty", fnum.format(rs.getFloat("FQty")+rs.getFloat("TLFQty")));
//				dic.put("TSqty", fnum.format(rs.getFloat("CLQty")+rs.getFloat("DJQty")-rs.getFloat("FQty")-rs.getFloat("TLFQty")));
//				rows.add(dic);
            }
    		sb.deleteCharAt(sb.length()-1);
    		sb.append("]}");
...
//        db.Execute("select * into tmp_Scheduling  from  #TmpScheduling");
        
        //销售订单物料需求信息
        sql = " select a.ErpNo, a.FInterID, a.FBOMNumber, a.FItemID, b.FName, b.FNumber, b.FModel, a.FAuxQty, a.FQty "
            + " into #TmpErpMtInfo "
            + " from( "
            + "         select b.ErpNo, a.FBOMNumber, a.FInterID, a.FItemID, "
            ...
            + "         group by b.ErpNo, a.FBOMNumber, a.FInterID, a.FItemID "
            + " )a "
        ...
        db.Execute("select * into tmp_ErpMtInfo  from  #TmpErpMtInfo");
        
        //插入物料需求里可互替的XXX或XXR物料而又不在需求里的XXX或XXR物料
        /* sql = " insert into #TmpErpMtInfo(ErpNo, FInterID, FBOMNumber, FItemID, FName, FNumber, FModel, FAuxQty, FQty) "
            + " select b.ErpNo, b.FInterID, b.FBOMNumber, a.FItemID, a.FName, a.FNumber, a.FModel, 0 as FAuxQty, 0 as FQty "
            + " from( "
            + "         select a.* "
            + "         from( "
        ...
        //插入替代物料
        sql = "insert into #TmpErpMtInfo(ErpNo, FInterID, FBOMNumber, FItemID, FName, FNumber, FModel, FAuxQty, FQty) "
        		+ "select a.ErpStr, 0, '',b.FItemID, b.FName, b.FNumber, b.FModel, a.NeedNum as FAuxQty, a.NeedNum as  FQty "
